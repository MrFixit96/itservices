...
##########Checking for invalid/dangerous characters
   ...
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the name field. Please click Post another request and start over.\n";
   exit;}
elsif($assetNumber=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the Asset Number field. Please click Post another request and start over.\n";
   exit;}
elsif ($errorType=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the errorType field. Please click Post another request and start over.\n";
   exit;}
elsif ($email=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the email field. Please click Post another request and start over.\n";
   exit;}
elsif ($priorityLevel=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the Priority Level field. Please click Post another request and start over.\n";
   exit;}
elsif ($description=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the description field. Please click Post another request and start over.\n";
   exit;}
elsif ($resolution=~ /;|&|>|<|\?|\!|\'|\`/){
   print "<a href= ../../services/index.php?Department=25>Back To IT Request System Home</a> "," <a href=\"", $request->url(), "?action=post\">Post Another Request</a>", "<br />" ;
   print "Special Characters such as ; & > < ? ! ' ` are not allowed in the resolution field. Please click Post another request and start over.\n";
   exit;}
}
#########################################################################################
#	submit_entry									#
...
########Submit data to the database via sql
    ...
    my $script = "INSERT INTO error_tracking ( Name, Email, AssetTag, Priority, Status, ErrorType, Description, OS, TimeStart) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    my $sth = $dbh->prepare($script);

    #Execute script filling in placeholders with variables passed to the execute function
    $sth->execute(( $name, $email, $assetNumber, $priorityLevel, $status, $errorType, $description, $OS, $timeStart));
    $sth->finish();
